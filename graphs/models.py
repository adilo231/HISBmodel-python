...
def HISBmodel(Graph, Seed_Set, Statistical, Opinion_Set, Beta_min=0.2, Beta_max=1.4, Omega_min=0.1, Omega_max=6, Delta_min=0.2, Delta_max=1.6, Jug_min=0.1, Jug_max=1):
    ...
    OpinionSupporting = 0
    RumorPopularity = 0
    Graph = InitParameters(Graph, Beta_min, Beta_max, Omega_min,
                           Omega_max, Delta_min, Delta_max, Jug_min, Jug_max)
    ...
    # ------------------------------
    Statistical.append({'NonInfected': Nbr_nonInfected, 'Infected': Nbr_Infected, 'Spreaders': Nbr_Spreaders,
                       'OpinionDenying': OpinionDenying, 'OpinionSupporting': OpinionSupporting, 'RumorPopularity': RumorPopularity, 'graph': Graph})
    # ----------------------
    ...
    while ListInfectedNodes:
        ...
        for X in reversed(range(0, L)):
            ...
            if np.exp(-RelativeTime * Graph.nodes[id]['beta']) < 0.2:
                ...
                # atrraction of nodes
                ActualAttraction = np.exp(-RelativeTime * Graph.nodes[id]['beta']) * np.abs(
                    np.sin((RelativeTime * Graph.nodes[id]['omega']) + Graph.nodes[id]['delta']))
                ...
                if np.random.random_sample() <= ActualAttraction:
                    ...
                    success = np.random.uniform(
                        0, 1, len(Graph.nodes[id]['neighbors'])) < Probability  # choic alpha nodes
                    new_ones = list(np.extract(
                        success, sorted(Graph.nodes[id]['neighbors'])))
                    Graph.nodes[id]['SendR'] += len(new_ones)
                    # Sending Rumor
                    for each in new_ones:
                        # Accepted Rumor Probability
                        AccepR = Graph.nodes[id]['degre'] / (
                            Graph.nodes[id]['degre'] + Graph.nodes[each]['degre'])*0.4
                    ...
                    if Graph.nodes[id]['opinion'] == "denying":
                        ...
                        OpinionSupporting -= 1
                    Graph.nodes[id]['opinion'] = updateOpinion(
                        jug=Graph.nodes[id]['jug'], Accpet_NegR=Graph.nodes[id]['Accp_NegR'], Nbr_OF_R=Graph.nodes[id]['AccpR'])
                    if Graph.nodes[id]['opinion'] == "denying":
                        OpinionDenying += 1
                    else:
                        OpinionSupporting += 1
        ...
        Statistical.append({'NonInfected': Nbr_nonInfected, 'Infected': Nbr_Infected, 'Spreaders': Nbr_Spreaders,
                           'OpinionDenying': OpinionDenying, 'OpinionSupporting': OpinionSupporting, 'RumorPopularity': RumorPopularity, 'graph': 0})
        time += 0.125
...
def graphe_TO_json(g):
    ...
    data = json_graph.node_link_data(
        g, {"link": "links", "source": "source", "target": "target", "weight": "weight"})
    data['nodes'] = [{"id": i, "state": "non_infected", "opinion": "normal", "beta": 0, "omega": 0, "delta": 0, "jug": 0, "Infetime": 0, "AccpR": 0, "SendR": 0,
                      "Accp_NegR": 0, "value": 0, "infected": 'false', "degre": g.degree[i], "neighbors":[n for n in g.neighbors(i)]} for i in range(len(data['nodes']))]
...
x1 = []
Lable = []
g = json_graph.node_link_graph(Small_World_networks(nbrofnodes))
HISBmodel(g, [1, 2, 3, 8, 12], Statistical, ['supporting',
          'supporting', 'denying', 'denying', 'supporting'], 0.2, 0.3)
for i in range(0, len(Statistical)):
    y1.append(Statistical[i]['Infected'])
    y2.append(Statistical[i]['Spreaders'])
    yT.append(Statistical[i]['Infected']/10)
...
y1.clear()
Lable.append("Test "+str(0.1))
lengthList = len(Statistical)
Beta = 0.3
for j in range(1, 10):
    print(j)
    y1 = []
    ...
    Statistical = []
    g = json_graph.node_link_graph(Small_World_networks(nbrofnodes))
    HISBmodel(g, [1, 2, 3, 8, 12], Statistical, [
              'supporting', 'supporting', 'denying', 'denying', 'supporting'], Beta, Beta+(j/10))

    if lengthList > len(Statistical):
        for i in range(0, len(Statistical)):
            y1.append(Statistical[i]['Infected'])
            y2.append(Statistical[i]['Spreaders'])
            yT[i] += y1[i]/10
        for i in range(len(Statistical), lengthList):
            y1.append(Statistical[len(Statistical)-1]['Infected'])
            yT[i] += y1[i]/10
    else:
    ...
    x1 = range(0, lengthList)

    plt.plot(x1, y1)

    y1.clear()
    Lable.append("Test "+str(Beta+(j/10)))

#quotients = [yT / 10 for number in yT]
plt.plot(x1, yT)
Lable.append("last ")
plt.legend(Lable)
